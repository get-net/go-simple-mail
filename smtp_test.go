...
`
	...
	t.Run("ehlo checkpoint", func(t *testing.T) {
		const (
			basicServer = `250-mx.google.com at your service
250-SIZE 35651584
250-CHECKPOINT
...
`
			...
			basicClient = `EHLO localhost
MAIL FROM:<user@gmail.com> TRANSID=<23.test.email@gmail.com> SIZE=50000
QUIT
`
		)
		...
		if ok, _ := c.extension("CHECKPOINT"); !ok {
			t.Fatalf("Should support CHECKPOINT")
		}
		if ok, _ := c.extension("SIZE"); !ok {
			t.Fatalf("Should support SIZE")
		}
		...
		if ok, _ := c.extension("CHECKPOINT"); ok {
			cmdArgs["CHECKPOINT"] = "23.test.email@gmail.com"
		}
